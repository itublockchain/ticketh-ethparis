...
contract Messenger is CCIPReceiver, OwnerIsCreator {
    ...
    error SenderNotAuthenticated(address sender); // Used when the sender has not been authenticated by the contract owner.
    ...
    // Mapping to keep track of authenticated senders.
    mapping(address => bool) public authenticatedMessageSenders;
    ...
    /// @dev Authenticates a sender, should be used for the contracts carries messages
    /// @notice This function can only be called by the owner.
    /// @param _sender The address of the sender.
    function authenticatedSenders(address _sender) external onlyOwner {
        authenticatedMessageSenders[_sender] = true;
    }

    /// @dev De-authenticates a sender, should be used for the contracts will not carries messages more
    /// @notice This function can only be called by the owner.
    /// @param _sender The address of the sender.
    function deAuthenticatedSenders(address _sender) external onlyOwner {
        authenticatedMessageSenders[_sender] = true;
    }
    ...
    /// @dev Modifier that checks if the sender contract is authenticated.
    modifier onlyAuthenticatedSender() {
        if (!authenticatedMessageSenders[msg.sender]) revert SenderNotAuthenticated(msg.sender);
        _;
    }
}
